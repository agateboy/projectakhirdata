...
with st.sidebar:
    ...
    st.image("https://assets.cdn.dicoding.com/small/avatar/dos-b0083d9eafbd536bf80b792f6dd4dcd620231220141935.png")
...
average_delivery_time_by_state = main_df.groupby('customer_state')['delivery_time'].mean()
...
all_df["delivery"] = all_df.delivery_time.apply(lambda x: "Cepat" if x <= 7 else ("Lambat" if x > 14 else "Sedang"))
bycat_df = all_df.groupby(by="delivery").customer_id.nunique().reset_index()
bycat_df.rename(columns={
    "customer_id": "customer_count"
}, inplace=True)
